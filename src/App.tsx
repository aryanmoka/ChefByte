...
function App() {
  ...
  useEffect(() => {
    try {
      ...
      // If localStorage unavailable (private mode?), still create a session id
      const fallback = `session_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`;
      setSessionId(fallback);
    }
  }, []);

  // Close mobile menu when clicking outside (backdrop already closes on click, but this keeps a11y tidy)
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-orange-50 dark:bg-gray-900 transition-colors duration-300 flex flex-col font-inter">
      ...
      {/* ===== Replace header + overlay + mobile sidebar with this block ===== */}
<header className="fixed w-full top-0 left-0 z-30" style={{}}>
  <div className="bg-white dark:bg-gray-800 shadow-md p-4 flex justify-between items-center max-w-full mx-auto rounded-b-xl">
    ...
    <nav className="hidden md:flex space-x-6">
      <button
        ...
        className={`text-gray-700 dark:text-gray-300 hover:text-indigo-600 dark:hover:text-amber-300 transition duration-200 font-medium py-2 px-3 rounded-md ${!hasStartedChat && !showContactPage ? 'text-indigo-600 dark:text-amber-300 font-bold' : ''}`}
      >
        Home
      </button>
      <button
        onClick={() => navigateTo('contact')}
        className={`text-gray-700 dark:text-gray-300 hover:text-indigo-600 dark:hover:text-amber-300 transition duration-200 font-medium py-2 px-3 rounded-md ${showContactPage ? 'text-indigo-600 dark:text-amber-300 font-bold' : ''}`}
      >
        Contact
      </button>
    </nav>
...
>
  <div className="h-full flex flex-col bg-white dark:bg-gray-900 border-l border-gray-200 dark:border-gray-800 shadow-xl">
    ...
    <nav className="px-4 py-2 space-y-1 flex-1 overflow-auto">
      ...
      <button
        onClick={() => { setIsMobileMenuOpen(false); navigateTo('chatbot'); }}
        className="block w-full text-left py-3 px-3 rounded-md text-gray-800 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 transition"
      >
        Chat
      </button>
...
{/* ===== end replacement ===== */}
      ...
      <main id="main-content" className="pt-[64px] md:pt-[64px] flex-grow">
